...
RoomManager::RoomManager(){
    ...
    int room_initialized[] = {101,102,103,104,105,106,
...
bool RoomManager::RoomInfo(const int& room)
{
    ...
    if(guess != NULL){
        ...
        UI::displayMessage("lich su checkin - checkout");
        UI::displayMessage("thoi gian\tcheckin/out");
        UI::displayMessage(guess->getCheckin() + "\tcheckin");
        UI::displayMessage(guess->getCheckout() + "\tcheckout");
        UI::displayMessage("Feedback: " + feedback(*guess));
        UI::displayMessage("--------------------------------------");
        return SUCCESS;
    }
    return FAIL;
...
void RoomManager::deleteRoom(const int& number){
   ...
   if(list_room_available.empty()){
        UI::displayMessage("chưa có phòng nào được thêm");
        return;
   }
   //xóa phòng ra khỏi danh sách sẵn sàng sử dụng
   if(EraseAvailableRoom(number) == FAIL){
        UI::displayMessage("Phòng đang có khách sử dụng");
   }
   //vô hiệu hóa phòng 
   Activate_Deactivate(number,DEACTIVATE);
   UI::displayMessage("xóa phòng" + to_string(number) + " thành công");
}