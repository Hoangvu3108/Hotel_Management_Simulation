...
bool File_Handle::SaveData(const char* path,Storage_dtype infotype){
    ...
    case GUESS_CSV:
        if(list_guess.empty())
        {
            UI::displayMessage("không có thông tin khách");
            goto error_handle;
        } 
        SaveGuessInfo(file);
        break;
    case ROOM_CSV:
        ...
        {
            UI::displayMessage("không có thông tin phòng");
            goto error_handle;
        }
        SaveRoomInfo(file);
        break;
    case SERVICE_CSV:
        ...
        {
            UI::displayMessage("không có thông tin dịch vụ");
            goto error_handle;
        }
    ...
    case EMMPLOYEE_CSV:
        ...
        {
            UI::displayMessage("không có thông tin nhân viên");
            goto error_handle;
        }
...
void File_Handle::LogRoomInfo(char *line){
    ...
    room[number] = static_cast<available_room_status>(status); 
