...
string GuessEmployee:: GenerateRandomService(){
    ...
    return list_service_name[number]; //tra ve mo ta dich vu tuong ung
}
...
void GuessEmployee::ServiceFeedbackInfo(const Guess& guess){
    ...
    auto getPrice = [guess](){
         for(auto& service : list_service){
            for(auto& service_ : service.getService()){
                if(service_.first == GYM && (guess.getService()) == list_service_name[1]) return service_.second;
                else if(service_.first == BIRTHDAY_PARTY && (guess.getService()) == list_service_name[2]) return service_.second;
                else if(service_.first == CAR_PARKING   && (guess.getService()) == list_service_name[3]) return service_.second;
                else if(service_.first == PET_CARE && (guess.getService()) == list_service_name[4]) return service_.second;
                else if(service_.first == FOOD_ORDER && (guess.getService()) == list_service_name[5]) return service_.second;
                else if(service_.first == SPA_MASSAGE && (guess.getService()) == list_service_name[6]) return service_.second;
    ...
    UI::displayMessage("dịch vụ sử dụng");
    UI::displayMessage("Tên\t\t\tĐơn giá\t\t\tsố lượng");
    UI::displayMessage(guess.getService() + "\t\t|" + to_string(price) + "\t\t\t|" +  to_string(quantity));
    UI::displayMessage("Thời gian ở: " + to_string(day_count));
    UI::displayMessage("Tổng chi phí dịch vụ: " + to_string(raw_cost) + " $");
    UI::displayMessage("Thuế VAT: 10%");
    UI::displayMessage("feedback: " + list_service_name());    
    UI::displayMessage("Thành tiền: " + to_string(final_cost) + " $");
}
Guess* GuessEmployee::GuessInfo(const string& phone){
    Guess* guess = findRoombyPhone(phone);
    if(guess != NULL){
        UI::displayMessage("Thong tin khach");
        UI::displayMessage("ten: " + guess->getName());
        UI::displayMessage("sdt: " + guess->getPhone());
        UI::displayMessage("phong o: " + to_string(guess->getRoom()));
        UI::displayMessage("thoi gian checkIn: " + guess->getCheckin());
...
bool GuessEmployee::ShowListGuess(){
    ...
    for(auto& guess : list_guess){
        UI::displayMessage("So phong: " + to_string(guess.getRoom()) + 
                        ",ten: " + guess.getName() + 
...
bool GuessEmployee::ShowListRoomAvailable(){
    ...
    auto status = [](available_room_status st){
    ...
    //in ra tầng đầu tiên
    UI::displayMessage("floor " + to_string(floor));
    ...
    for(size_t i = 0 ; i < list_room_available.size() ; i++){
        ...
        available_room_status st;             //trạng thái của phòng 
        ...
        if((next - current) > 5){
            UI::displayMessage("\t" + to_string(current) + "(" + status(st) + ")");   // in ra tâng phòng cuối của tầng hiện tại
