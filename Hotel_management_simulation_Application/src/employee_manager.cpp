...
bool EmployeeManager::addEmployee(vector<Account> &accountList, const string &name, const string &phoneNumber, const string &position)
{
    ...
    {
        UI::displayMessage("Số điện thoại này đã được đăng ký.");
        return FAILURE;
    }
    ...
    UI::displayMessage("Nhân viên " + name + " với ID " + id + " đã được thêm thành công.");
    ...
    UI::displayMessage("Tài khoản cho nhân viên đã được đăng ký với mật khẩu mặc định " + generateRandompassword());
    return SUCCESS;
}
...
void EmployeeManager::editEmployeeById(const string &id, const string &newName, string &newPosition)
{
    ...
    if (employee)
    {
        ...
        UI::displayMessage("Thông tin nhân viên đã được cập nhật.");
    }
    else
    {
        UI::displayMessage("Không tìm thấy nhân viên với tên này.");
    }
}
...
void EmployeeManager::deleteEmployeeById(const string &id)
{
    ...
    if (it != list_employee.end())
    {
        ...
        UI::displayMessage("Nhân viên với ID " + id + " đã được xóa.");
    }
    else
    {
        UI::displayMessage("Không tìm thấy nhân viên với ID này.");
    }
}
...
void EmployeeManager::setWorkShift(const string &id, const string &shift)
{
    ...
    if (employee == nullptr)
    {
        UI::displayMessage("Không tìm thấy nhân viên với ID: " + id);
        return;
    }

    if(setWorkshiftfromString(*employee, shift))
    {
        UI::displayMessage("Ca làm việc của nhân viên " + employee->getName() + " đã được cập nhật thành công thành: " + shift);
    }
    else{
...
void EmployeeManager::listEmployees() const
{
    ...
    {
        UI::displayMessage("Không có nhân viên nào.");
    }
    else
    {
...
void EmployeeManager::ShowWorkshiftTable() const
{
    if (list_employee.empty())
    {
        UI::displayMessage("Chưa có nhân viên");
    }
    else
    {
